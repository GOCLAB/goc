...
BOOST_FIXTURE_TEST_CASE( get_scope_test, TESTER ) try {
   ...
   create_accounts({ N(gocio.token), N(gocio.ram), N(gocio.ramfee), N(gocio.stake),
      N(gocio.bpay), N(gocio.vpay), N(gocio.saving), N(gocio.names),
      N(gocio.gns), N(gocio.gstake), N(gocio.vs) });
   ...
   push_action(N(gocio.token), N(create), N(gocio.token), act );

   // issue
   for (account_name a: accs) {
      push_action( N(gocio.token), N(issue), "gocio", mutable_variant_object()
                  ("to",      name(a) )
   ...
   eosio::chain_apis::read_only::get_table_by_scope_params param{N(gocio.token), N(accounts), "inita", "", 10};
   ...
   if (result.rows.size() >= 4) {
      BOOST_REQUIRE_EQUAL(name(N(gocio.token)), result.rows[0].code);
      BOOST_REQUIRE_EQUAL(name(N(inita)), result.rows[0].scope);
      BOOST_REQUIRE_EQUAL(name(N(accounts)), result.rows[0].table);
      BOOST_REQUIRE_EQUAL(name(N(gocio)), result.rows[0].payer);
