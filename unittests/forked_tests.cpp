...
BOOST_AUTO_TEST_CASE( forking ) try {
   ...
   auto r2 = c.create_accounts( {N(gocio.token)} );
   wdump((fc::json::to_pretty_string(r2)));
   c.set_code( N(gocio.token), eosio_token_wast );
   c.set_abi( N(gocio.token), eosio_token_abi );
   c.produce_blocks(10);


   auto cr = c.push_action( N(gocio.token), N(create), N(gocio.token), mutable_variant_object()
              ("issuer",       "gocio" )
   ...
   cr = c.push_action( N(gocio.token), N(issue), config::system_account_name, mutable_variant_object()
              ("to",       "dan" )
