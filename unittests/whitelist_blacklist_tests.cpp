...
class whitelist_blacklist_tester {
   public:
      ...
      void init( bool bootstrap = true ) {
         ...
         chain->create_accounts({N(gocio.token), N(alice), N(bob), N(charlie)});
         chain->set_code(N(gocio.token), eosio_token_wast);
         chain->set_abi(N(gocio.token), eosio_token_abi);
         chain->push_action( N(gocio.token), N(create), N(gocio.token), mvo()
              ( "issuer", "gocio.token" )
              ( "maximum_supply", "1000000.00 TOK" )
         );
         chain->push_action( N(gocio.token), N(issue), N(gocio.token), mvo()
              ( "to", "gocio.token" )
      ...
      transaction_trace_ptr transfer( account_name from, account_name to, string quantity = "1.00 TOK" ) {
         return chain->push_action( N(gocio.token), N(transfer), from, mvo()
            ( "from", from )
            ( "to", to )
            ( "quantity", quantity )
            ( "memo", "" )
...
BOOST_AUTO_TEST_CASE( actor_whitelist ) { try {
   ...
   test.actor_whitelist = {config::system_account_name, N(gocio.token), N(alice)};
   test.init();

   test.transfer( N(gocio.token), N(alice), "1000.00 TOK" );
...
BOOST_AUTO_TEST_CASE( contract_whitelist ) { try {
   whitelist_blacklist_tester<> test;
   test.contract_whitelist = {config::system_account_name, N(gocio.token), N(bob)};
   test.init();

   test.transfer( N(gocio.token), N(alice), "1000.00 TOK" );

   test.transfer( N(alice), N(gocio.token) );
...
BOOST_AUTO_TEST_CASE( action_blacklist ) { try {
   whitelist_blacklist_tester<> test;
   test.contract_whitelist = {config::system_account_name, N(gocio.token), N(bob), N(charlie)};
...
BOOST_AUTO_TEST_CASE( blacklist_onerror ) { try {
   ...
   BOOST_CHECK_EXCEPTION( tester1.chain->produce_blocks(), fc::exception,
                          fc_exception_message_is("action 'gocio::onerror' is on the action blacklist")
                        );
