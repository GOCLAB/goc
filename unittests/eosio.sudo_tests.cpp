...
public:
   ...
   eosio_sudo_tester() {
      create_accounts( { N(gocio.msig), N(prod1), N(prod2), N(prod3), N(prod4), N(prod5), N(alice), N(bob), N(carol) } );
      produce_block();
      ...
      set_code( N(gocio.sudo), eosio_sudo_wast, &system_private_key );
      set_abi( N(gocio.sudo), eosio_sudo_abi, &system_private_key );
      ...
      const auto& accnt = control->db().get<account_object,by_name>( N(gocio.sudo) );
      abi_def abi;
      BOOST_REQUIRE_EQUAL(abi_serializer::to_abi(accnt.abi, abi), true);
      abi_ser.set_abi(abi, abi_serializer_max_time);

      while( control->pending_block_state()->header.producer.to_string() == "gocio" ) {
         produce_block();
      }
   }

   void propose( name proposer, name proposal_name, vector<permission_level> requested_permissions, const transaction& trx ) {
      push_action( N(gocio.msig), N(propose), proposer, mvo()
                     ("proposer",      proposer)
   ...
   void approve( name proposer, name proposal_name, name approver ) {
      push_action( N(gocio.msig), N(approve), approver, mvo()
                     ("proposer",      proposer)
                     ("proposal_name", proposal_name)
   ...
   void unapprove( name proposer, name proposal_name, name unapprover ) {
      push_action( N(gocio.msig), N(unapprove), unapprover, mvo()
                     ("proposer",      proposer)
                     ("proposal_name", proposal_name)
...
BOOST_FIXTURE_TEST_CASE( sudo_exec_direct, eosio_sudo_tester ) try {
...

   {
      ...
      sudo_trx.actions.emplace_back( get_action( N(gocio.sudo), N(exec),
                                                 {{N(alice), config::active_name}, {N(gocio.sudo), config::active_name}},
                                                 mvo()
                                                   ("executer", "alice")
                                                   ("trx", trx)
      ) );
   ...
   BOOST_REQUIRE_EQUAL( "gocio", name{trace->action_traces[0].act.account} );
...
BOOST_FIXTURE_TEST_CASE( sudo_with_msig, eosio_sudo_tester ) try {
   ...
   push_action( N(gocio.msig), N(exec), N(alice), mvo()
                  ("proposer",      "carol")
                  ("proposal_name", "first")
                  ("executer",      "alice")
   );
   ...
   BOOST_REQUIRE_EQUAL( "gocio.sudo", name{traces[0]->action_traces[0].act.account} );
   ...
   BOOST_REQUIRE_EQUAL( "gocio", name{traces[1]->action_traces[0].act.account} );
...
BOOST_FIXTURE_TEST_CASE( sudo_with_msig_unapprove, eosio_sudo_tester ) try {
   ...
   BOOST_REQUIRE_EXCEPTION( push_action( N(gocio.msig), N(exec), N(alice), mvo()
                                          ("proposer",      "carol")
                                          ("proposal_name", "first")
                                          ("executer",      "alice")
