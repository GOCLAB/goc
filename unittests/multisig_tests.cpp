...
public:
   ...
   eosio_msig_tester() {
      create_accounts( { N(gocio.msig), N(gocio.stake), N(gocio.ram), N(gocio.ramfee), N(alice), N(bob), N(carol) } );
      produce_block();
   ...
   asset get_balance( const account_name& act ) {
      ...
      const auto* tbl = db.find<table_id_object, by_code_scope_table>(boost::make_tuple(N(gocio.token), act, N(accounts)));
      share_type result = 0;
   ...
   transaction_trace_ptr push_action( const account_name& signer, const action_name& name, const variant_object& data, bool auth = true ) {
      ...
      auto trace = base_tester::push_action( N(gocio.msig), name, accounts, data );
      produce_block();
...
BOOST_FIXTURE_TEST_CASE( update_system_contract_all_approve, eosio_msig_tester ) try {
   ...
   set_authority(config::system_account_name, "active", authority(1,
      vector<key_weight>{{get_private_key("gocio", "active").get_public_key(), 1}},
   ...
   BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"),
                        get_balance("gocio") + get_balance("gocio.ramfee") + get_balance("gocio.stake") + get_balance("gocio.ram") );
...
BOOST_FIXTURE_TEST_CASE( update_system_contract_major_approve, eosio_msig_tester ) try {
   ...
   // set up the link between (gocio active) and (gocio.prods active)
   set_authority(config::system_account_name, "active", authority(1,
      vector<key_weight>{{get_private_key("gocio", "active").get_public_key(), 1}},
      vector<permission_level_weight>{{{config::producers_account_name, config::active_name}, 1}}), "owner",
      { { config::system_account_name, "active" } }, { get_private_key( config::system_account_name, "active" ) });
