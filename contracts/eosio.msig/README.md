...
Create a proposal
## gocio.msig::propose    proposer proposal_name requested trx
...
Approve a proposal
## gocio.msig::approve    proposer proposal_name level
   - **proposer** account proposing a transaction
...
## gocio.msig::unapprove    proposer proposal_name level
   - **proposer** account proposing a transaction
   - **proposal_name** name of the proposal
...
Cancel a proposal
## gocio.msig::cancel    proposer proposal_name canceler
   - **proposer** account proposing a transaction
   - **proposal_name** name of the proposal
   - **canceler** account canceling the transaction (only proposer can cancel not expired transaction)

Execute a proposal
## gocio.msig::exec    proposer proposal_name executer
   - **proposer** account proposing a transaction
   - **proposal_name** name of the proposal
...
Prerequisites:
   - eosio.token contract installed to gocio.token account, eosio.msig contract installed on gocio.msig account which is a privileged account.
...
````
$ cleos multisig propose test '[{"actor": "treasury", "permission": "active"}]' '[{"actor": "treasury", "permission": "active"}]' gocio.token issue '{"to": "tester", "quantity": "1000.0000 EOS", "memo": ""}' -p tester
executed transaction: e26f3a3a7cba524a7b15a0b6c77c7daa73d3ba9bf84e83f9c2cdf27fcb183d61  336 bytes  107520 cycles
#    gocio.msig <= gocio.msig::propose          {"proposer":"tester","proposal_name":"test","requested":[{"actor":"treasury","permission":"active"}]...
````
...
#    gocio.msig <= gocio.msig::approve          {"proposer":"tester","proposal_name":"test","level":{"actor":"treasury","permission":"active"}}
````
...
#    gocio.msig <= gocio.msig::exec             {"proposer":"tester","proposal_name":"test","executer":"tester"}
````
